...
* **Friend Search:** Users can search by username prefix. Search results expose only safe fields: `user_id`, `username`, `avatar_url`, and relationship state (`none`, `pending_incoming`, `pending_outgoing`, `accepted`). Search must not expose score totals, habit metadata, logs, journal text, or private messages. The Flutter search row should make `pending_outgoing` visibly revocable in place, while `pending_incoming` remains informational and directs the user back to the Friends tab for accept/decline.
...
* **Friend Profile Privacy:** `GET /api/social/user/:id/profile` must stay authenticated and accepted-friend scoped. It may return only safe identity fields, compact backend-owned achievement metadata, lifetime score/level metadata, and allowed active-habit summaries (`title`, optional `description`, `color_hex`, `target_duration`, `current_duration`, role context). It must not return journal text, raw logs, or unrestricted third-party social graph data. `Follow` in Flutter pre-fills local habit creation and does not create remote follow state.
